...
def lambda_handler(event, context):
    try:
        ...
        # ---------- Update posts-table with input fields ----------
        # Only include fields that are not None (DynamoDB will accept nulls, but we can keep explicit)
        expression_attrs = {}
        expression_names = {}
        update_expr = []
        for k, v in update_fields.items():
            ...
            expression_attrs[placeholder_value] = v if v is not None else None
        ...
        # ---------- Fetch Post ----------
        post_resp = posts_table.get_item(Key={"post_id": post_id})
        if "Item" not in post_resp:
            return response(404, "Post not found")

        post_item = post_resp["Item"]
        campaign_id = post_item.get("campaign_id")
        ...
        posts_table.update_item(
            ...
            ExpressionAttributeValues={":st": current_time, ":status": "published"}
        )
        ...
        for img in image_keys:
            ...
            upload_res = http.request(
                ...
                upload_url,
                body=image_bytes,
                headers={"Authorization": f"Bearer {access_token}", "Content-Type": mimetype}
            )
