...
def read_text_from_s3(s3_path: str):
    """
    Reads a text file from S3 and returns (success, content_or_error_message).
    """
...
def build_s3_path_from_key(key: str, variable: str, doc: str):
    parts = key.split("/")
    if len(parts) != 2:
        raise ValueError("Invalid key format. Expected format: 'folder/filename'")
    ...
    for item in inputs.split(","):
        item = item.strip()
        # Replace "output/" with the S3 path containing the variable
    ...
    # Prepend the template S3 URI
    input_list.insert(0, template_s3_uri)

    return input_list


def build_s3_path_from_key_old(key: str):
    parts = key.split("/")
    if len(parts) != 2:
        raise ValueError("Invalid key format. Expected format: 'folder/filename'")

    folder, filename = parts
    template_s3_uri = f"s3://cammi-devprod/prompt/{key}/{filename}.txt"
    input_s3_uri = f"s3://cammi-devprod/input/{key}/{filename}.txt"

    template_bucket, template_key = _parse_s3_uri(input_s3_uri)
    inputs = _read_s3_object(template_bucket, template_key)
    input_list = [item.strip() for item in inputs.split(",")]
    input_list.insert(0, template_s3_uri)
...
def build_prompt(s3_uris: list) -> str:
    ...
    return merge_using_template(template, replacements)
...
# ---------- Main Function ----------
def bedrock_from_s3_files_converse(result: str, doc: str, key: str, s3_uris: list[str], model_id: str, max_tokens: int, temperature: float, session_id: str, project_id: str, additional_prompt: str = "") -> dict:
    base_prompt = build_prompt(s3_uris)

    print("this is prompt",base_prompt)
...
# Define document types that should NOT include formatting_instruction
    EXCLUDED_DOC_TYPES = ["qmp", "cc"]

# Add formatting_instruction only for document types NOT in the exclusion list
    if doc not in EXCLUDED_DOC_TYPES:
        base_prompt = f"{formatting_instruction}{base_prompt.strip()}"

# Then build final prompt based on whether additional_prompt is provided
...
Ensure the new version is targeted, and aligned with the original input above and instructions."""
        ...
        # ✅ Generic formatting instruction wrapper
...
# Goal: Refine the selected section to better reflect the client’s expectations while maintaining alignment with the original brief and format.
# Task Description:  
# Below is client feedback on the previous response genereated: {additional_prompt.strip()}.
# Ensure the new version is targeted, and aligned with the original input above and instructions."""
#     else:
        
#         prompt = f"{formatting_instruction}{base_prompt.strip()}"
    ...
    try:
        ...
        return {
            "error": f"Error invoking model: {str(e)}",
            "session_id": session_id
        }
...
def lambda_handler(event, context):
    try:
        ...
        event["document_type"] = document_type
        event["result"] = result  # include prompt + response + cost
